...
**IS/OOS validation (2026-09-04, FIRST RUN — RED, tightening applied):**
IS 2022-23: 587 trades (275 mm_golden), WR 41.7%, PF 3.42, **MaxDD -21.96%** (breaches -15%).
OOS 2024-25: GREEN — MaxDD -7.79%, PF 4.17, +186 trades. The model has genuine edge; the IS
failure is correlated dollar exposure: the de-correlation gate only checked other mm_golden
trades, NOT base strategy positions. EU base long + GU golden short = double dollar exposure.

**Tightening (2026-09-05):** two fixes applied together:
1. **Cross-model de-correlation** (`MM_GOLDEN_DECORR_ALL=1`, default ON): blocks mm_golden when
   ANY active position (base or golden) has the same dollar direction (`-direction` for all
   X/USD pairs). Previously only checked `entry_model == "mm_golden"`.
2. **Draw score minimum ≥ 2** (`MM_GOLDEN_MIN_DRAW=2`, default 2): requires at least 2 of 3
   HTF timeframes (W/D/H4) to agree with the trade. Previously only blocked draw_score == 0.

**Status:** tightened, compiles clean, default OFF. Needs IS/OOS re-validation with tightened
gates: `MM_GOLDEN_ENABLED=1 python run_backtest_histdata.py --years 2022 2023` and
`--years 2024 2025`. Ships only if both splits stay positive PF and MaxDD holds ≤ -15%.

### P47 — Narrative context scoring (BUILT 2026-09-05, analytics-only conviction contributor)
**What:** contextual "story" layer for WHY a move should happen. Five factors, each contributing
0 or 1 to conviction (total 0-5). Analytics-only on first deployment — proves the signal in
IS/OOS analytics tables before any gate or sizing lever. Same measurement-first discipline as
P9/P15/P16/P19.

**Module:** `ict/narrative.py` — `NarrativeContext` class, pure-logic, no backtest state.

**Factors:**
1. **ICT Weekly Profile** (`narrative_weekly_profile`): replaces the old DOW tendency (Tue/Wed
   always). Context-dependent day roles using `weekly_amd_dir` from `detect_weekly_amd`, covering
   three ICT weekly archetypes (Classic Expansion, Consolidation Reversal, Midweek Reversal).
   - **Tuesday**: +1 always (the key manipulation day — all profiles agree)
   - **Monday/Wednesday**: +1 when no weekly AMD yet (manipulation/range-building window —
     Monday IS tradeable per user directive), +1 when weekly AMD confirms direction, 0 when
     AMD opposes
   - **Thursday**: +1 when weekly AMD confirms direction (distribution / Consolidation Reversal),
     0 when no AMD (no profile confirmation)
   - **Friday**: +1 when weekly AMD confirms direction (late-profile expansion — Consolidation
     Reversal 5/5 + Midweek Reversal 4.5/5), 0 when no AMD or opposing (Classic Expansion TGIF)
   Config: `NARRATIVE_WEEKLY_PROFILE_ENABLED=1`. Old `NARRATIVE_DOW_DAYS` removed.
2. **NFP week consolidation** (`narrative_nfp`): Mon/Tue of NFP week — tight accumulation range
   = high-quality Judas sweep. +1 on Mon/Tue of NFP week. Wed-Fri already gated by existing
   `is_nfp_week_low_probability`.
3. **Rate decision context** (`narrative_rate`): day-after-FOMC/ECB/BOE = post-decision
   continuation (+1). FOMC day itself = whipsaw context (-1, penalizes conviction). Otherwise 0.
4. **Prior-session PD array provenance** (`narrative_pd_prov`): previous session's AMD sweep
   direction agrees with current trade direction → narrative is "continuation of established
   distribution." +1 when previous sweep aligns.
5. **Seasonal/monthly tendency** (`narrative_seasonal`): monthly directional lean per pair from
   `data/seasonal_bias.json` (generated by `scripts/build_seasonal.py`). +1 when month's tendency
   (grade ≥ "moderate") agrees with trade direction. Gracefully handles missing file.

**Integration:** conviction contributor in both `_maybe_open` and `_mm_golden_entry`.
`weekly_amd_dir` is passed to `NarrativeContext.score()` — already computed in `_maybe_open`
(line ~2797); now also computed in `_mm_golden_entry` via `_get_weekly_amd`. Expected to be
saturation-inert on first deployment (same as P9/P15/P16/P19 — trades already at conviction > 4).
The value is in the analytics columns proving whether narrative context predicts trade quality.

**Trade record columns:** `narrative_score` (int 0-5), `narrative_weekly_profile`, `narrative_nfp`,
`narrative_rate`, `narrative_pd_prov`, `narrative_seasonal`.

**Reporting:** "Narrative context scoring (P47)" table by score (WR/PF) + per-factor fired vs
absent breakdown (WR/PF each). In both console output and `data/backtest_report.md`.
