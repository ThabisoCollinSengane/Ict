...
**Factors:**
1. **DOW tendency** (`narrative_dow`): Tue/Wed are ICT's strongest distribution days. +1 on
   days in `NARRATIVE_DOW_DAYS` (default "1,2" = Tue/Wed, Mon=0).
...
**Integration:** conviction contributor in both `_maybe_open` and `_mm_golden_entry`. Expected
to be saturation-inert on first deployment (same as P9/P15/P16/P19 — trades already at
conviction > 4). The value is in the analytics columns proving whether narrative context predicts
trade quality.

**Trade record columns:** `narrative_score` (int 0-5), `narrative_dow`, `narrative_nfp`,
